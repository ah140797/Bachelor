...
We load Libraries
```{r}
pacman::p_load(tidyverse, wesanderson, patchwork, psych, xlsx, RColorBrewer, GPArotation, ggcorrplot, ggpubr, bestNormalize)
```
...
f2_plot <- loadings_fa_long %>% 
  ...
  ggplot() +
  aes(x = order, y = loading) +
  geom_bar(aes(fill = loading), stat = 'identity', width = 0.4) +
  labs(x='', y='Factor loadings')+
  #geom_text(axis.text.x = element_text(angle = 90,size=12,hjust=1)) +
  scale_fill_distiller(palette = 'Blues',direction = 1)+# Add categories to axis
  scale_x_continuous(
...
png("figures/exploratory_factor_analysis/loadings_depr.png", width = 10, height = 10, units = 'in', res = 300)
f1_plot
dev.off()

png("figures/exploratory_factor_analysis/loadings_anx.png", width = 10, height = 10, units = 'in', res = 300)
f2_plot
dev.off()
# +
...
corr_var <- round(cor(data_fa_2_cor %>% select('Factor 1: Depression', 'Factor 2: Anxiety', PHQ9, GAD7, EmoStable)), 2)
